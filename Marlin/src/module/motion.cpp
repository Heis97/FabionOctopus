...
xyz_pos_t home_offset_Z{0};
xyz_pos_t home_offset_A{0};
xyz_pos_t home_offset_B{0};
#if HAS_HOME_OFFSET && HAS_POSITION_SHIFT
  // The above two are combined to save on computes
  xyz_pos_t workspace_offset{0};
#endif
xyz_pos_t workspace_offset_Z{0};
xyz_pos_t workspace_offset_A{0};
xyz_pos_t workspace_offset_B{0};
#if HAS_ABL_NOT_UBL
...
void prepare_line_to_destination() {
   ...
   */
  void do_homing_move(AxisEnum axis,  float distance, feedRate_t fr_mm_s=0.0,  bool final_approach=true) {
...
void do_homing_move_laser(AxisEnum axis,  float distance, feedRate_t fr_mm_s=0.0,  bool final_approach=true) {
      // Get the ABC or XYZ positions in mm
    abce_pos_t target = planner.get_axis_positions_mm();
    ...
    //planner.set_machine_position_mm(target);                 // The move will be towards the endstop
    planner.buffer_segment(target OPTARG(HAS_DIST_MM_ARG, cart_dist_mm), fr_mm_s, active_extruder);
    planner.synchronize();
   // planner.set_machine_position_mm(planner.get_axis_positions_mm());
  }
  /**
...
void update_workspace_offset_Z(int32_t z_n) {
  if (z_n == 0)
  {
    workspace_offset_Z[X_AXIS] = home_offset_Z[X_AXIS] + position_shift_Z[X_AXIS] + petry_shift[X_AXIS];
    workspace_offset_Z[Y_AXIS] = home_offset_Z[Y_AXIS] + position_shift_Z[Y_AXIS] + petry_shift[Y_AXIS];
    workspace_offset_Z[Z_AXIS] = home_offset_Z[Z_AXIS] + position_shift_Z[Z_AXIS] + petry_shift[Z_AXIS];
    if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("Axis ", AS_CHAR(AXIS_CHAR(axis)), " home_offset = ", home_offset_Z[axis], " position_shift = ", position_shift_Z[axis]);
  }

  if (z_n == 1)
  {
    workspace_offset_A[X_AXIS] = home_offset_A[X_AXIS] + position_shift_A[X_AXIS] + petry_shift[X_AXIS];
    workspace_offset_A[Y_AXIS] = home_offset_A[Y_AXIS] + position_shift_A[Y_AXIS] + petry_shift[Y_AXIS];
    workspace_offset_A[I_AXIS] = home_offset_A[I_AXIS] + position_shift_A[I_AXIS] + petry_shift[I_AXIS];
    if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("Axis ", AS_CHAR(AXIS_CHAR(axis)), " home_offset = ", home_offset_A[axis], " position_shift = ", position_shift_A[axis]);
  }

  if (z_n == 2)
  {
    workspace_offset_B[X_AXIS] = home_offset_B[X_AXIS] + position_shift_A[X_AXIS] + petry_shift[X_AXIS];
    workspace_offset_B[Y_AXIS] = home_offset_B[Y_AXIS] + position_shift_B[Y_AXIS] + petry_shift[Y_AXIS];
    workspace_offset_B[J_AXIS] = home_offset_B[J_AXIS] + position_shift_B[J_AXIS] + petry_shift[J_AXIS];
    if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("Axis ", AS_CHAR(AXIS_CHAR(axis)), " home_offset = ", home_offset_B[axis], " position_shift = ", position_shift_B[axis]);
  }
...
void set_home_offset_Z(const AxisEnum axis, const float v,int32_t z_n) {
    if (z_n == 0)
    {
       home_offset_Z[axis] = v;
    }
    if (z_n == 1)
    {
       home_offset_A[axis] = v;
    }
    if (z_n == 2)
    {
       home_offset_B[axis] = v;
    }    
    update_workspace_offset_Z(z_n);
  } 
